...
- **boundaries**:
  ...
  - **飛行制御に触る PR** では、branch 確定・merge 推薦の前に **bato の独立監査を必ず挟む** (スキップする裁量を持たない)。**飛行制御に触らない PR** (harness / 集計器 / guard / docs) では独立監査を必須としない —— 必須なのは `/pr-gate` と機械検証 (CLAUDE.md PR 前ゲート項 3 / 4) である。**どちらの経路かを決めるのは motoko で、分類の根拠を担当者に開示する** (誤分類を下から指摘できる形にするため)
...
```
飛行制御に触る    実装 → 自己 pr-gate → bato 独立監査 → motoko 裁定 → operator 判断 (merge 等)
触らない          実装 → 自己 pr-gate → 機械検証 (項 3) → motoko 裁定 → operator 判断
```

どの段も次の段をスキップさせる裁量を持たない。**どちらの経路かは motoko が決め、分類の根拠を担当者に開示する。**

**由来 (2026-08-15 の実測)**: 非・飛行制御の PR 1 本で、`/pr-gate` が 12 件、独立監査が 1 件、`make test-integration` の事前予測が 2 件を出した。**監査が見逃した 2 件は、いずれも機械検証が落とすもの**だった。加えて、統括が渡した観点に監査範囲が縛られることが control で実測された (既知の 2 件を意図的に渡さず監査に出したところ、2 件とも検出されなかった)。監査の価値が無いのではなく、**非・飛行制御では機械検証のほうが確実に落とす**。
...
13. **実装タスクの起点は常に `/ticket <ID>` とする。着手済みのチケットでも同じで、着手済みである旨を指示文に明記する。ID は上流が指定し、下流は推定しない** (operator 裁定 2026-08-02、2026-08-15 に「一番最初か」の判定を撤廃)

    - **起点を分岐させない。** 2026-08-02 版は「その課題に着手する一番最初か」で `/ticket` と直接指示を分けていたが、この判定自体を撤廃した。分岐が無ければ、motoko が判定を誤る余地も無い
    - **着手済みのチケットに `/ticket` を使うときは、着手済みである旨と現在地を指示文に書く** (Linear の status、既存 branch の有無、既に落ちている成果)。`/ticket` は入口のコマンドなので、入口以外で使うと下記のずれ 1 が必ず当たる
    - **作業が operator から降りてくるとき: operator がチケットを指定する。** motoko は「どのチケットの話か」を推定しない。ID は作業が何であるかを知っている側から出る
    - **作業が team 内で発生したとき** (発見された欠陥 / PR 是正 / handoff の継続 / SITL 運転 / 監査): motoko は指示文に **「`/ticket <ID>` で始めてください」** と書く。ID が未採番なら先に起票する。補足情報 (背景・制約・先行する監査結果) は同じ便で渡してよい
    ...
    - **既知のずれ (2026-08-02 時点、未解決)** — saito が `.claude/commands/ticket.md` 706 行を実読して検出。**`/ticket` を課題の入口以外で使うと規律違反に着地しうる**:
        1. **branch 作成が新規前提** (§4 `git checkout -b`)。既存 branch を resume する分岐が無い ← **常時 `/ticket` にしたので、着手済みチケットでは必ず当たる。**指示文に既存 branch 名を書き、新規に切らないと明示する
...
17. **上げた escalation が未回答のまま滞留していることを、待ち手の生存に依存せずに検出できる形にする** (tachikoma 指摘、2026-08-15)

    共通ルール 10 は「待ちが 30 分を超えたら報告する」を定めるが、**その担い手は待っている本人の session である**。session が落ちれば止まる。そして統括の側も同じく揮発性で、2026-08-15 に tachikoma の escalation 2 件が NEX2-2896 の往復に埋もれ、回答の無いまま流れかけた。**拾い直せたのは待ち手の自己申告があったからで、統括は気づいていなかった。**

    **待ち手と統括の両方が揮発性で、どちらも同じ理由で失敗する。**

    - **担い手を 2 つに分けて書く** (tachikoma 指摘。分けずに書くと後者だけ実装して前者を忘れる):

      | 担い手 | 役割 |
      | :--- | :--- |
      | 何が待ちかを知っている側 | 未回答の escalation を保持し、内容を言える |
      | 時間を数える側 | 一定時間の経過で発火する |

      **時間は数えられるが何が待ちか言えない機構は、滞留を検出できない。**

    - **統括は escalation を上げたら、同じ便で Linear に台帳を書く。** agmsg と各 session の context は揮発するが、Linear は残る。どちらの session が落ちても、次にその issue を開いた人が滞留を見つけられる。台帳には **内容・上げた時刻・状態** を書く (実例: NEX2-2910 の「未回答の escalation 台帳」)
    - **止めるときは Linear の status も戻す** (tachikoma 指摘)。台帳を置いても、status が実態とずれていれば開いた人は誤読する
    - **待ち手は共通ルール 10 の自己申告を続ける。** 台帳は滞留を **見つけられる** ようにするだけで、**見に行く人** は依然として必要である。二重にする

    **機械検証は無い。** 台帳を書き忘れたことを落とす仕組みは存在しない。ここに担い手を置けないことを承知のうえで、慣行として記す (共通ルール 15 と同じ扱い)。

## 変更履歴

- 2026-08-15 (同日 4 度目): 共通ルール 17 を追加 (operator 承認、tachikoma 指摘) — escalation の滞留検出を待ち手の生存に依存させない。統括が Linear へ台帳を書く形を最低限の担い手とし、**機械検証が無いことを本文に明記**した。併せて motoko boundaries と検証ゲートの標準フローを改訂 (operator 裁定) — **bato の独立監査を飛行制御に触る PR に限定**し、触らない PR は `/pr-gate` + 機械検証で足りるとした。同日の実測 (監査 1 件 / pr-gate 12 件 / 機械検証が見逃し 2 件を捕捉、かつ統括の brief が監査範囲を縛ることが control で判明) が由来

- 2026-08-15 (同日 2 度目): 共通ルール 13 を改訂 (operator 裁定) — 起点の判定を撤廃し、**着手済みのチケットでも常に `/ticket <ID>` から始める**形へ。着手済みである旨と現在地 (Linear の status / 既存 branch / 既に落ちている成果) を指示文に書くことで代える。併せて既知のずれ 1 (branch 新規前提) を「入口で使う限り問題にならない」から「着手済みでは必ず当たる」へ改めた
